...
<body class="bg-gray-50 min-h-screen flex items-center justify-center">
    <div class="container bg-white shadow-lg rounded-lg p-8">
        <header class="flex justify-between items-center border-b pb-4 mb-6">
            <h2 class="form-heading">Create New Entry</h2>
        ...
        <form method="POST" action="{{ route('store') }}" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" id="name" name="name" class="form-input @error('name') border-red-500 @enderror" placeholder="Enter your full name" value="{{ old('name') }}">
                    @error('name')
                ...
                <div>
                    ...
                    <input type="text" id="adress" name="adress" class="form-input @error('adress') border-red-500 @enderror" placeholder="Enter your address" value="{{ old('adress') }}">
                    @error('adress')
                        <p class="error-message">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="number" id="age" name="age" class="form-input @error('age') border-red-500 @enderror" placeholder="Enter your age" value="{{ old('age') }}">
                    @error('age')
                        <p class="error-message">{{ $message }}</p>
                    @enderror
                </div>
